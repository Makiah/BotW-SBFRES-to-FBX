...
def openAndWriteToLog(thingToWrite: str):
    logFile = open(os.path.join(fbxExtractionPath, "log.txt"), "a")
    logFile.write(thingToWrite + "\n")
    logFile.close()
...
if not os.path.exists(modelLoadingWorkspace):
    ...
    openAndWriteToLog("Had to create model loading workspace!")
openAndWriteToMoreToLoad("true")
openAndWriteToLog("Reset true file")
...
for previousWorkspaceFile in os.listdir(modelLoadingWorkspace):
    if previousWorkspaceFile.endswith(".png") or previousWorkspaceFile.endswith(".bfres"):
        ...
        openAndWriteToLog("Removed " + previousWorkspaceFile + " from workspace!")
...
for root, dirs, files in os.walk(bfresDatabase):
    for modelSubdirectory in dirs:
        ...
        openAndWriteToLog("Checking for " + expectedFBX)
        if not os.path.exists(expectedFBX):
            # Output that it currently does not exist.
            openAndWriteToLog("Doesn't exist!  Exporting now.  ")
            ...
            for bfresOrTextureFile in os.listdir(exportedBfresSubdirectory):
                ...
                openAndWriteToLog("Added " + bfresOrTextureFile + " to the workspace!")
...
openAndWriteToLog("No more to add!")
